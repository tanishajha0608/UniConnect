...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.user) {
        // Get university from email domain and redirect to university dashboard
        const universitySlug = getUniversityFromEmail(formData.email)
        if (universitySlug) {
          router.push(`/${universitySlug}/dashboard`)
        } else {
          // Fallback to landing page if university can't be determined
          router.push("/")
        }
      }
    } catch (error) {
